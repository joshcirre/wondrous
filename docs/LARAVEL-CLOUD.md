...
The app is prepared for Laravel Cloud, but no deployment has been created from this workspace.

1. Connect this repository to a Cloud application. Use PHP8.4 and Node22+ (Node24 LTS recommended).
...
4. Deploy command: `php artisan migrate --force`. Set APP_ENV=production, APP_DEBUG=false, APP_KEY (generate once), APP_URL to the HTTPS application domain. Keep SESSION_DRIVER=database, CACHE_STORE=database and QUEUE_CONNECTION=database unless adding shared Redis. Use SESSION_SECURE_COOKIE=true.
...
6. Run a queue worker: `php artisan queue:work --sleep=1 --tries=3 --timeout=60`. Websocket invalidations are queued after commits; without the worker the UI will fall back to polling but won't receive live invalidations.
...
Scheduler API reference: [Laravel scheduling frequencies](https://api.laravel.com/docs/12.x/Illuminate/Console/Scheduling/ManagesFrequencies.html).
